...
const getClassName = prefixClassName('rsc-slide');
...
const Slide = React.forwardRef<SlideRefType, Props>((props, ref) => {
  ...
  const slideToPageIndex = useCallback(
    (newPageIndex: number, transition = true) => {
      ...
      if (el) {
        const { offsetWidth: wrapWidth, offsetHeight: wrapHeight } =
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const fg = new Touch(el, {
      ...
      onTouchEnd: () => {
        ...
        if (
          direction === 'horizontal' &&
          Math.abs(thisRef.current.x - thisRef.current.lastX) >
            wrapWidth * ratio
        ) {
          slideToPageIndex(
            pageIndex + (thisRef.current.x < thisRef.current.lastX ? 1 : -1),
          );
        } else if (
          direction === 'vertical' &&
          Math.abs(thisRef.current.y - thisRef.current.lastY) >
            wrapHeight * ratio
        ) {
          slideToPageIndex(
            pageIndex + (thisRef.current.y < thisRef.current.lastY ? 1 : -1),
          );
        } else {
          // reset
      ...
      onPressMove: (e) => {
        ...
        if (direction === 'horizontal') {
          if (
            thisRef.current.x > 0 ||
            thisRef.current.x < -1 * (count - 1) * wrapWidth
          ) {
            return;
          }
          ...
          if (
            thisRef.current.y > 0 ||
            thisRef.current.y < -1 * (count - 1) * wrapHeight
          ) {
            return;
          }
